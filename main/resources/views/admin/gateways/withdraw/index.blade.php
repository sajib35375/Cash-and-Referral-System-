                ...
                <div class="card h-100 text-center">
                    <div class="card-body px-3">
                        ...
                        <div class="d-flex gap-2 flex-wrap justify-content-center">
                            <a href="{{ route('admin.withdraw.method.edit', $method->id) }}" class="btn rounded-pill btn-label-primary">
                                <span class="tf-icons las la-pen me-1"></span> @lang('Edit')
                            </a>

                            @if ($method->status)
                                <button class="btn rounded-pill btn-label-warning decisionBtn" data-question="@lang('Are you confirming the deactivation of this method?')" data-action="{{ route('admin.withdraw.method.status', $method->id) }}">
                                    <span class="tf-icons las la-ban me-1"></span>
                                    @lang('Inactive')
                                </button>
                            @else
                                <button class="btn rounded-pill btn-label-success decisionBtn" data-question="@lang('Are you confirming the activation of this method?')" data-action="{{ route('admin.withdraw.method.status', $method->id) }}">
...
@push('breadcrumb')
    <a href="{{ route('admin.withdraw.method.new') }}" class="btn rounded-pill btn-label-primary">
